...
test('Upload a file', async test => {
	test.plan(1)
	const file = await new Files()
	try {
		const upload = await file.add(
			{
				Name: '5001 Brief',
				Description: 'CW bief',
				account: 3,
				filePath: '/tmp/upload_f1d877351fc4ad999f946b4ce9a7628b',
				fileName: '5001CEM SEM1 CW Brief - Google Docs.pdf',
				fileType: 'application/pdf'
			})
		test.is(upload, true, 'unable to upload')
	} catch (err) {
		test.fail('error thrown')
	} finally {
		file.close()
